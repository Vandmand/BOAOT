...
class Node {
    constructor(name, parent, priority, level) {
        this.name = name; // Name of node. Used to reference the node in the tree
        this.parent = parent; // The node's parent. For easier access through the tree
        this.data = new Map(); // Data of the node. Contains other nodes and leafs
        this.properties = [] // Properties (where all actual funktions are stored)
...
export const renderList = new Node('ROOT', 'NONE', 0, 0);
...
function insertNode(node) {
    const parent = node.parent == 'NONE' ? renderList : node.parent;
...
export function render(node = renderList) {
    push();
...
export const debug = {
    logTree: () => {
        ...
        let level = "-"
        function loop(node = rd.renderList) {
